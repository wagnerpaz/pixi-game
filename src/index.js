...
window.onresize = function () {
  app.renderer.resize(
    window.innerWidth / PIXI.settings.RESOLUTION,
    window.innerHeight / PIXI.settings.RESOLUTION
  );
};
window.onresize();
...
async function load() {
  ...
  app.stage.x = 0;
  app.stage.y =
    hero.x - SCROLL_AREA_HEIGHT - hero.height / 2 - app.stage.height / 2;

  const scrollArea = new PIXI.Graphics();
  scrollArea.visible = false;
  scrollArea.lineStyle(1, 0xff0000);
  scrollArea.drawRect(0, 0, SCROLL_AREA_WIDTH, SCROLL_AREA_HEIGHT);
  ...
  app.ticker.add((delta) => {
    ...
    if (hero.x <= scrollArea.x && app.stage.x < 0) {
      ...
      hero.x + hero.width >= scrollArea.x + scrollArea.width &&
      app.stage.x + app.stage.width > -app.stage.width
    ) {
      scrollArea.x += nxu;
      app.stage.x -= nxu;
    }
    ...
    if (hero.y <= scrollArea.y && app.stage.y < 0) {
      ...
      hero.y + hero.height >= scrollArea.y + scrollArea.height &&
      app.stage.y + app.stage.height > -app.stage.height
    ) {
      scrollArea.y += nyu;
      app.stage.y -= nyu;
    }
